...
### [GAS&#x2011;29] Multiple accesses of the same mapping/array key/index should be cached
...
Gas saved per Instance: ~42 _(Total: ~966)_
...
/// @audit totals[token] is also accessed on line 105
/// @audit totals[token] is also accessed on line 108
/// @audit totals[token] is also accessed on line 112
/// @audit caps[token] is also accessed on line 114
114:         if (caps[token] > 0 && totals[token].total > caps[token]) {

/// @audit balances[msg.sender][token] is also accessed on line 106
/// @audit balances[msg.sender][token] is also accessed on line 109
118:         balances[msg.sender][token].total += amount;

/// @audit balances[msg.sender][token] is also accessed on line 124
125:         balances[msg.sender][token].locked += amount;

/// @audit totals[token] is also accessed on line 126
127:         totals[token].locked += amount;

/// @audit balances[msg.sender][token] is also accessed on line 133
134:         balances[msg.sender][token].total -= amount;

/// @audit totals[token] is also accessed on line 135
136:         totals[token].total -= amount;
```

[114](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/blast/BlastOnboarding.sol/#L114-L114), [118](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/blast/BlastOnboarding.sol/#L118-L118), [125](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/blast/BlastOnboarding.sol/#L125-L125), [127](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/blast/BlastOnboarding.sol/#L127-L127), [134](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/blast/BlastOnboarding.sol/#L134-L134), [136](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/blast/BlastOnboarding.sol/#L136-L136)

```solidity
📁 File: src/blast/BlastOnboardingBoot.sol

/// @audit balances[user] is also accessed on line 162
162:         uint256 userLocked = balances[user][MIM].locked + balances[user][USDB].locked;
```

[162](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/blast/BlastOnboardingBoot.sol/#L162-L162)

```solidity
📁 File: src/mimswap/periphery/Factory.sol

/// @audit _pools[poolIndex] is also accessed on line 128
131:         _pools[poolIndex] = _pools[_pools.length - 1];

/// @audit _userPools[userPoolIndex] is also accessed on line 134
138:         _userPools[userPoolIndex] = _userPools[_userPools.length - 1];
```

[131](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/mimswap/periphery/Factory.sol/#L131-L131), [138](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/mimswap/periphery/Factory.sol/#L138-L138)

```solidity
📁 File: src/mimswap/periphery/Router.sol

/// @audit path[iterations] is also accessed on line 561
563:             amountOut = IMagicLP(path[iterations]).sellQuote(to);
```

[563](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/mimswap/periphery/Router.sol/#L563-L563)

```solidity
📁 File: src/staking/LockingMultiRewards.sol

/// @audit _rewardData[rewardToken] is also accessed on line 279
/// @audit _rewardData[rewardToken] is also accessed on line 282
/// @audit _rewardData[rewardToken] is also accessed on line 283
285:         return _rewardData[rewardToken].rewardPerTokenStored + pendingRewardsPerToken;

/// @audit _rewardData[rewardToken] is also accessed on line 305
314:         _rewardData[rewardToken].exists = true;

/// @audit _rewardData[rewardToken] is also accessed on line 362
369:         Reward storage reward = _rewardData[rewardToken];

/// @audit lastLockIndex[user] is also accessed on line 417
/// @audit lastLockIndex[user] is also accessed on line 430
431:                 lastLockIndex[user] = index;

/// @audit locks[index] is also accessed on line 422
/// @audit locks[index] is also accessed on line 426
435:                 locks[index] = locks[lastIndex];

/// @audit lastLockIndex[user] is also accessed on line 482
502:             lastLockIndex[user] = lockCount;

/// @audit _userLocks[user] is also accessed on line 483
/// @audit _userLocks[user] is also accessed on line 490
/// @audit _userLocks[user] is also accessed on line 501
/// @audit _userLocks[user][_lastLockIndex] is also accessed on line 490
510:             _userLocks[user][_lastLockIndex].amount += amount;

/// @audit _rewardData[token_] is also accessed on line 532
533:         _rewardData[token_].lastUpdateTime = uint248(lastTimeRewardApplicable_); // safe to cast as this will never overflow

/// @audit rewards[user][rewardToken] is also accessed on line 616
/// @audit rewards[user] is also accessed on line 616
619:             rewards[user][rewardToken] = 0;

/// @audit _userRewardLock[user] is also accessed on line 598
648:                 _userRewardLock[user].items.push(RewardLockItem({token: rewardToken, amount: rewardAmount}));
```

[285](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/staking/LockingMultiRewards.sol/#L285-L285), [314](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/staking/LockingMultiRewards.sol/#L314-L314), [369](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/staking/LockingMultiRewards.sol/#L369-L369), [431](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/staking/LockingMultiRewards.sol/#L431-L431), [435](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/staking/LockingMultiRewards.sol/#L435-L435), [502](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/staking/LockingMultiRewards.sol/#L502-L502), [510](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/staking/LockingMultiRewards.sol/#L510-L510), [533](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/staking/LockingMultiRewards.sol/#L533-L533), [619](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/staking/LockingMultiRewards.sol/#L619-L619), [648](https://github.com/code-423n4/2024-03-abracadabra-money/blob/main/src/staking/LockingMultiRewards.sol/#L648-L648)
